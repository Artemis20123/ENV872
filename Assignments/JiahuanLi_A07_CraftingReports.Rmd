...
```{r project.setup, message=FALSE, warning=FALSE, echo=TRUE, results='hide'}
#Import libraries
...
```{r,echo=FALSE,warning=FALSE,fig.width=8,fig.height=3.5,fig.align='left',fig.cap="Monthly Ozone, 2018"}
epa_data %>% 
  filter(Year == 2018) %>% 
  ggplot(
    aes(
      ...
      y = Ozone)) +
  geom_boxplot() +
  scale_x_discrete(labels= month.abb[unique(epa_data$Month)]) +
  xlab("Month") + 
  ylab(expression("Ozone levels ( "*mu*g/m^3*" )")) +
  ggtitle("Recorded Ozone by Month (2018)")
```

```{r,echo=FALSE,warning=FALSE,fig.width=8,fig.height=3.5,fig.align='left',fig.cap="Monthly Ozone, 2019"}
epa_data %>% 
  filter(Year == 2019) %>% 
  ggplot(
    aes(
...
Add a table of contents to your document and knit to a PDF.

```{r table of contents}
# use the `toc` chunk option in the YAML header
# should pay attention to the format and spacing
```

Submit your PDF to Sakai, but also be sure to commit and push your Rmd file used to create this knit document to GitHub. In the section below, add a link to your GitHub repository.

### Git Repository

<https://github.com/Artemis20123/ENV872/tree/main/Assignments>
