...
# just find some values from the matrix
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[c(12, 14, 3)] #

# designate specific rows and columns, slice of matrix
matrix1[2:4, 1:3] #
...
# designate the format (2*2) of vector storage
...
e <- c("red", "white", "red", NA) # What type of vector? four elements
...
rownames(dataframe1) <- c("v1","v2","v3","v4")
...
```{r DataExercise-1a}
high_F = c(64, 71, 72, 63, 51, 52, 61, 54, 67, 73)
low_F = c(42, 43, 60, 46, 32, 32, 41, 47, 52, 54)
...
```{r DataExercise-1c}
forecast = data.frame(high_F, low_F, high_c, low_c)
names(forecast) = c("highF","lowF","highC","lowC")
```
...
# keyboard shortcuts can be viewed and modified in Tools and Help
```
...
```{r DateExercise2}
#century issue: 1955 shown as 2055
str_past <- "55-feb-3"
...
fixed_date_obj_past <- fix.early.dates(date_obj_past,cutoff=23) #cutoff could be the current year to be more general or any other depending on data set. this year is 2023
fixed_date_obj_past
```
