...
#Option 1 (one is df, the other is vector format, will end up in df)
df <- cbind(df_car_speed,car_dist)  #similarly, rows could be added using rbind() (that is adding new rows to df)
class(df)
df
...
summary(df)
head(df, 10) #show the first 6 (default) rows of df

# looking for help
??head

#If you know the number of the column you want you can refer to that to access column/value/slice of origin df
vector = df[,1]
value = df[10,1]
new_df = df[10:20, 1:2]
new_df
...
## usually do not do that. just creat new smaller df
df2 <- df[,-2]
df2
```
