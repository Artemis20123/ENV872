...
## alternative
NTL.phys.data.EX1 <- filter(NTL.phys.data, year(sampledate) == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tues <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
...
# default dataframe is listed in the first line, thus, do not need to repeat it in the pipes

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July.
NTL.phys.data.processed1 <-
  NTL.phys.data %>%
...
summary(NTL.phys.data.processed2)

## difference between summarize() and summary()
## summarize() is an operation and yield new dataframe containing statistics
## summary() does not change anything in the original dataset, but provide statistics for the dataframe
```
